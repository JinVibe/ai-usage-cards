...
const dataCache = new TtlCache<UsageData>(10 * 60 * 1000);
...
export default async function handler(req: VercelRequest, res: VercelResponse): Promise<void> {
  ...
  try {
    const data = await dataCache.getOrCompute(gistId.toLowerCase(), () =>
      fetchUsageData(gistId, { token: process.env.GITHUB_TOKEN }),
    );
    const svg =
      data.days.size === 0
